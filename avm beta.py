...
if st.session_state['password_approved']:
    ...
    st.title("AVM Beta - Point Analysis")
    ...
    if click_data and 'last_active_drawing' in click_data and click_data['last_active_drawing']:
        ...
        if not (button_predict and latitude and longitude and lebar_jalan_di_depan and luas_tanah):
            ...
            req = requests.post("https://apiavm.rhr.co.id/predict", json=request_data, verify = False)
            ...
            if req.status_code == 200:
                ...
                # sosio_analysis = requests.get("https://apiavm.rhr.co.id/geo/get-demography", params = {"lon" : longitude, "lat" : latitude}, verify= False)
                # st.write(sosio_analysis.status_code)
    ...
    st.error("Password is incorrect or not provided. Please try again.")
    # st.stop()
